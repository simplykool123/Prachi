/*
  ...
  ## Tables Created
  1. **products** - Inventory product master with SKU, category, pricing, stock
  2. **stock_movements** - Tracks all stock in/out movements
  3. **suppliers** - Supplier master with contact and ledger
  4. **purchase_orders** - PO header
  5. **purchase_order_items** - PO line items
  6. **purchase_entries** - Actual purchase/GRN entries
  7. **purchase_entry_items** - Purchase entry line items
  8. **customers** - Customer master (B2B/B2C)
  9. **sales_orders** - Sales order header
  10. **sales_order_items** - Sales order line items
  11. **invoices** - Invoice header
  12. **invoice_items** - Invoice line items
  13. **delivery_challans** - Delivery challan header
  14. **delivery_challan_items** - Challan line items
  15. **payments** - Payment receipts/made
  16. **courier_entries** - Courier tracking
  17. **crm_notes** - Customer interaction notes
  18. **crm_files** - Customer file uploads
  19. **appointments** - Scheduling/calendar
  20. **travel_plans** - City-wise travel plans
  21. **expenses** - Business expense tracking
  22. **ledger_entries** - Double-entry accounting ledger
  23. **journal_entries** - Manual journal entries
  24. **automation_rules** - Rule-based automation config
  25. **whatsapp_logs** - WhatsApp message logs
...
CREATE TABLE IF NOT EXISTS products (
  ...
  name text NOT NULL,
  category text NOT NULL CHECK (category IN ('Astro Products', 'Vastu Items', 'Healing Items')),
...
CREATE TABLE IF NOT EXISTS payments (
  ...
  reference_type text NOT NULL CHECK (reference_type IN ('invoice', 'purchase_entry', 'advance')),
  reference_id uuid,
  customer_id uuid REFERENCES customers(id),
  supplier_id uuid REFERENCES suppliers(id),
...
CREATE TABLE IF NOT EXISTS crm_notes (
  ...
  note_type text DEFAULT 'Note' CHECK (note_type IN ('Note', 'Call', 'Meeting', 'Vastu Visit', 'Astro Reading', 'Follow Up')),
  title text NOT NULL,
...
CREATE TABLE IF NOT EXISTS crm_files (
  ...
  file_type text DEFAULT 'Document' CHECK (file_type IN ('Palm Image', 'Floor Plan', 'Report', 'Document', 'Photo')),
...
CREATE TABLE IF NOT EXISTS appointments (
  ...
  appointment_type text DEFAULT 'Consultation' CHECK (appointment_type IN ('Astro Reading', 'Vastu Audit', 'Consultation', 'Follow Up', 'Site Visit', 'Video Call', 'Phone Call')),
